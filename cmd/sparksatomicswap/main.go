...
// Copyright (c) 2019 The SparksPay developers
...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of SparksPay Core wallet RPC server")
...
// Sparks transactions for these swaps.  A second tool should be used for the
// transaction on the other chain.  Any chain can be used so long as it supports
// OP_SHA256 and OP_CHECKLOCKTIMEVERIFY.
//
// Example scenerios using Sparks as the second chain:
//
// Scenerio 1:
//   cp1 initiates (dcr)
//   cp2 participates with cp1 H(S) (Sparks)
//   cp1 redeems Sparks revealing S
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems dcr with S
//
// Scenerio 2:
//   cp1 initiates (Sparks)
//   cp2 participates with cp1 H(S) (dcr)
//   cp1 redeems dcr revealing S
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems Sparks with S

func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: sparksatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
type initiateCmd struct {
	cp2Addr *gosparksutil.AddressPubKeyHash
	amount  gosparksutil.Amount
}

type participateCmd struct {
	cp1Addr    *gosparksutil.AddressPubKeyHash
	amount     gosparksutil.Amount
	secretHash []byte
}
...
func run() (err error, showUsage bool) {
	...
	switch args[0] {
	case "initiate":
		cp2Addr, err := gosparksutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp2AddrP2PKH, ok := cp2Addr.(*gosparksutil.AddressPubKeyHash)
		if !ok {
		...
		amount, err := gosparksutil.NewAmount(amountF64)
		if err != nil {
			return err, true
		}
	...
	case "participate":
		cp1Addr, err := gosparksutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp1AddrP2PKH, ok := cp1Addr.(*gosparksutil.AddressPubKeyHash)
		if !ok {
...
// pubkey for a transaction input signature.  Due to limitations of the SparksPay
// Core RPC API, this requires dumping a private key and signing in the client,
// rather than letting the wallet sign.
func createSig(tx *wire.MsgTx, idx int, pkScript []byte, addr gosparksutil.Address,
...
// gosparks/rpcclient package.
func fundRawTransaction(c *rpc.Client, tx *wire.MsgTx, feePerKb gosparksutil.Amount) (fundedTx *wire.MsgTx, fee gosparksutil.Amount, err error) {
	...
	feeAmount, err := gosparksutil.NewAmount(resp.Fee)
	if err != nil {
		return nil, 0, err
	}
...
func getFeePerKb(c *rpc.Client) (useFee, relayFee gosparksutil.Amount, err error) {
	...
	relayFee, err = gosparksutil.NewAmount(netInfoResp.RelayFee)
	if err != nil {
		return 0, 0, err
	}
	payTxFee, err := gosparksutil.NewAmount(walletInfoResp.PayTxFee)
	if err != nil {
		return 0, 0, err
	}
	...
	if err == nil && estimateResp.FeeRate > 0 {
		useFee, err = gosparksutil.NewAmount(estimateResp.FeeRate)
		if relayFee > useFee {
			useFee = relayFee
		}
...
// account parameter which was removed in SparksPay Core 0.15.
func getRawChangeAddress(c *rpc.Client) (gosparksutil.Address, error) {
	...
	addr, err := gosparksutil.DecodeAddress(addrStr, chainParams)
	if err != nil {
		return nil, err
	}
...
type contractArgs struct {
	them       *gospakrsutil.AddressPubKeyHash
	amount     gosparksutil.Amount
	locktime   int64
	secretHash []byte
}
...
type builtContract struct {
	contract       []byte
	contractP2SH   gosparksutil.Address
	contractTxHash *chainhash.Hash
	contractTx     *wire.MsgTx
	contractFee    gosparksutil.Amount
	refundTx       *wire.MsgTx
	refundFee      gosparksutil.Amount
}
...
func buildContract(c *rpc.Client, args *contractArgs) (*builtContract, error) {
	...
	contractP2SH, err := gosparksutil.NewAddressScriptHash(contract, chainParams)
	if err != nil {
		return nil, err
	}
...
func buildRefund(c *rpc.Client, contract []byte, contractTx *wire.MsgTx, feePerKb, minFeePerKb gosparksutil.Amount) (
	refundTx *wire.MsgTx, refundFee gosparksutil.Amount, err error) {
	...
	refundAddr, err := gosparksutil.NewAddressPubKeyHash(pushes.RefundHash160[:], chainParams)
	if err != nil {
		return nil, 0, err
	}
	...
	if txrules.IsDustOutput(refundTx.TxOut[0], minFeePerKb) {
		return nil, 0, fmt.Errorf("refund output value of %v is dust", gosparksutil.Amount(refundTx.TxOut[0].Value))
	}
...
func calcFeePerKb(absoluteFee gosparksutil.Amount, serializeSize int) float64 {
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Contract fee: %v (%0.8f Sparks/kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.8f Sparks/kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	recipientAddr, err := gosparksutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	contractHash := gosparksutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if sc == txscript.ScriptHashTy &&
			bytes.Equal(addrs[0].(*gosparksutil.AddressScriptHash).Hash160()[:], contractHash) {
			contractOut = i
			break
		}
	}
	...
	if txrules.IsDustOutput(redeemTx.TxOut[0], minFeePerKb) {
		return fmt.Errorf("redeem output value of %v is dust", gosparksutil.Amount(redeemTx.TxOut[0].Value))
	}
	...
	fmt.Printf("Redeem fee: %v (%0.8f Sparks/kB)\n\n", fee, redeemFeePerKb)
...
func (cmd *auditContractCmd) runOfflineCommand() error {
	contractHash160 := gosparksutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if bytes.Equal(addrs[0].(*gosparksutil.AddressScriptHash).Hash160()[:], contractHash160) {
			contractOut = i
			break
		}
	}
	...
	contractAddr, err := gosparksutil.NewAddressScriptHash(cmd.contract, chainParams)
	if err != nil {
		return err
	}
	recipientAddr, err := gosparksutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
	...
	refundAddr, err := gosparksutil.NewAddressPubKeyHash(pushes.RefundHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	...
	fmt.Printf("Contract value:          %v\n", gosparksutil.Amount(cmd.contractTx.TxOut[contractOut].Value))
